...
class MockOntologyDAO extends OntologyDAO {
  val resources = List(TermResource(
    ...
    synonyms=List("primary central sleep apnea"),
    parents=List(
      TermParent(
      ...
      TermParent(
        ...
        synonyms=List("Non-organic sleep disorder")
      ),
      TermParent(
  ...
  override def search(term: String)(implicit ec: ExecutionContext, userInfo: UserInfo): Future[List[TermResource]] = Future(resources)
}
