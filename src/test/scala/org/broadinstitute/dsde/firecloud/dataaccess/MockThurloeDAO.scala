...
class MockThurloeDAO extends ThurloeDAO {
  ...
  override def getTrialStatus(forUserId: String, callerToken: WithAccessToken) = {
    ...
    Future.successful(
      forUserId match {
        case "enabled-user" =>
          Some(UserTrialStatus(forUserId, Some(TrialStates.Enabled), 0, 0, 0, 0, Some("date")))
        case "disabled-user" => None
        case _ =>
          Some(UserTrialStatus(forUserId, Some(TrialStates.Terminated), 0, 0, 0, 0, None))
      }
    )
  }
