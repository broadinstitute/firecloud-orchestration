...
class UserRegistrationSamDAO extends MockSamDAO {
  override def getRegistrationStatus(implicit userInfo: WithAccessToken): Future[RegistrationInfo] = {
    userInfo match {
      case x:UserInfo if x.id.equals(TRIAL_SELF_ENABLED_ERROR) =>
  ...
  override def registerUser(implicit userInfo: WithAccessToken): Future[RegistrationInfo] = {
    userInfo match {
      case x: UserInfo if x.id.equals(TRIAL_SELF_ENABLED_ERROR) => Future.successful(
        RegistrationInfo(
          WorkbenchUserInfo(x.id, x.userEmail),
          WorkbenchEnabled(google = true, ldap = true, allUsersGroup = true)))
      case _ => super.registerUser
    }
  }

  override def adminGetUserByEmail(email: RawlsUserEmail): Future[RegistrationInfo] = {
    email match {
      case x if x.value.equals(TRIAL_SELF_ENABLED_ERROR) => Future.successful(
        RegistrationInfo(
          WorkbenchUserInfo(x.value, x.value),
          WorkbenchEnabled(google = true, ldap = true, allUsersGroup = true)))
      case _ => super.adminGetUserByEmail(email)
    }
  }
