...
class MethodsServiceSpec extends ServiceSpec with MethodsService {
  ...
  override def beforeAll(): Unit = {
    ...
    httpMethods map {
      method =>
        methodsServer
          .when(request().withMethod(method.name).withPath(MethodsService.remoteMethodsUrl))
          .respond(
            org.mockserver.model.HttpResponse.response()
              .withHeaders(MockUtils.header).withStatusCode(OK.intValue)
          )
        methodsServer
          .when(request().withMethod(method.name).withPath(MethodsService.remoteConfigurationsUrl))
          .respond(
            org.mockserver.model.HttpResponse.response()
              .withHeaders(MockUtils.header).withStatusCode(OK.intValue)
          )
