...
class WorkspaceServiceSpec extends ServiceSpec with WorkspaceService with BeforeAndAfterEach {
  ...
  private final val bucketPath = workspacesRoot + "/%s/%s/bucket".format(workspace.namespace.get, workspace.name.get)
  ...
  "WorkspaceService Passthrough Negative Tests" - {
    ...
    "Passthrough tests on the /workspaces/segment/segment/bucket path" - {
      "MethodNotAllowed error is returned for HTTP POST, PATCH, PUT, DELETE methods" in {
        List(HttpMethods.POST, HttpMethods.PATCH, HttpMethods.PUT, HttpMethods.DELETE) map {
          method =>
            new RequestBuilder(method)("/workspaces/namespace/name/bucket") ~> sealRoute(routes) ~> check {
              status should equal(MethodNotAllowed)
            }
        }
      }
  ...
  "WorkspaceService Passthrough Tests" - {
    ...
    "Passthrough tests on the /workspaces/%s/%s/bucket path" - {
