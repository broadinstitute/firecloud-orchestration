...
class WorkspaceApiServiceSpec extends BaseServiceSpec with WorkspaceApiService with BeforeAndAfterEach {
  ...
  val realmRawlsWorkspaceResponse = WorkspaceResponse(WorkspaceAccessLevels.Owner, canShare=false, catalog=false, realmRawlsWorkspace, WorkspaceSubmissionStats(None, None, runningSubmissionsCount = 0), List.empty)
  val nonRealmedRawlsWorkspaceResponse = WorkspaceResponse(WorkspaceAccessLevels.Owner, canShare=false, catalog=false, nonRealmedRawlsWorkspace, WorkspaceSubmissionStats(None, None, runningSubmissionsCount = 0), List.empty)
  ...
  "WorkspaceService Passthrough Negative Tests" - {
    ...
    "Passthrough tests on the /workspaces/segment/segment/methodconfigs path" - {
      ...
      "Forbidden error is returned for HTTP POST with library attributes on entities in output" in {
        val methodConfigs = MethodConfiguration("namespace", "name", "root", Map.empty, Map.empty, Map("this.library:param" -> AttributeString("")), MethodRepoMethod("methodnamespace", "methodname", 1))
      ...
      "Forbidden error is returned for HTTP POST with library attributes on workspaces in output" in {
        val methodConfigs = MethodConfiguration("namespace", "name", "root", Map.empty, Map.empty, Map("workspace.library:param" -> AttributeString("")), MethodRepoMethod("methodnamespace", "methodname", 1))
        Post(methodconfigsPath, methodConfigs) ~> dummyUserIdHeaders("1234") ~> sealRoute(workspaceRoutes) ~> check {
          status should equal(Forbidden)
        }
      }
