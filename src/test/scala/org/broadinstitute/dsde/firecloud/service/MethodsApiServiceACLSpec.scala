...
class MethodsApiServiceACLSpec extends BaseServiceSpec with MethodsApiService {
  ...
  "MethodsServiceACLs" - {
      ...
      "MethodNotAllowed is returned" in {
        List(HttpMethods.DELETE, HttpMethods.PUT) map {
          method =>
            ...
            new RequestBuilder(method)("/" + localConfigPermissionsPath) ~> sealRoute(methodsServiceApiRoutes) ~> check {
              status should equal(MethodNotAllowed)
            }
        }
      }
    ...
    "when posting bad roles to methods" - {
      "BadRequest is returned" in {
        Post("/" + localMethodPermissionsPath, jsonBadRole) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
    ...
    "when posting bad roles to configs" - {
      "BadRequest is returned" in {
        Post("/" + localConfigPermissionsPath, jsonBadRole) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(BadRequest)
        }
      }
    }
    "when posting bad users to methods" - {
      "BadRequest is returned" in {
        Post("/" + localMethodPermissionsPath, jsonBadUser) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(BadRequest)
        }
      }
    }
    "when posting bad users to configs" - {
      "BadRequest is returned" in {
        Post("/" + localConfigPermissionsPath, jsonBadUser) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(BadRequest)
        }
      }
    }
    ...
    "when retrieving ACLs from configs" - {
      "the entire list is successfully translated" in {
        Get("/" + localConfigsPath + MockAgoraACLServer.standardPermsPath) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
    ...
    "when retrieving ACLs from methods" - {
      "the entire (reversed) list is successfully translated" in {
        Get("/" + localMethodsPath + MockAgoraACLServer.standardPermsPath) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(OK)
          var perms = responseAs[List[FireCloudPermission]]
    ...
    "when retrieving bad Agora data from configs" - {
      "InternalServerError is returned" in {
        Get("/" + localConfigsPath + MockAgoraACLServer.withEdgeCasesPath) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
    ...
    "when retrieving bad Agora data from methods" - {
      "InternalServerError is returned" in {
        Get("/" + localMethodsPath + MockAgoraACLServer.withEdgeCasesPath) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(InternalServerError)
        }
      }
    }
    ...
    "when posting good data to configs, expecting a good response" - {
      "a good response is returned" in {
        Post("/" + localConfigsPath + MockAgoraACLServer.standardPermsPath, standardFC) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(OK)
          var perms = responseAs[List[FireCloudPermission]]
          perms shouldBe standardFC
        }
    ...
    "when posting good data to methods, expecting an invalid response" - {
      "an invalid response is returned and we throw an error" in {
        Post("/" + localMethodsPath + MockAgoraACLServer.standardPermsPath, standardFC) ~> dummyAuthHeaders ~> sealRoute(methodsServiceApiRoutes) ~> check {
          status should equal(InternalServerError)
        }
      }
    }
