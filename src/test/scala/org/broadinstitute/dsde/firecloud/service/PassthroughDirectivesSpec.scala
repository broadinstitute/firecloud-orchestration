...
final class PassthroughDirectivesSpec extends BaseServiceSpec with HttpServiceBase
  ...
  private def validateAllPaths(path: String, queryParams: Option[Map[String, String]] = None)
                              (implicit ourTheirRoots: (String, String)): Unit = {
    validateAllPaths(path, path, queryParams)
  }

  private def validateAllPaths(inpath: String, outpath: String)
                              (implicit ourTheirRoots: (String, String)): Unit = {
  ...
  private def validateAllPathsWithRoots(inpath: String, outpath: String,
    ...
    val specRoute = {
      pathPrefix(separateOnSlashes(inroot)) {
        extract(_.request.method) { method =>
          unmatchedPath { remaining =>
            parameterMap { params =>
              passthrough(Uri(encodeUri(echoUrl + expectedOutRoot + remaining)).withQuery(params).toString, method)
            }
          }
        }
      }
